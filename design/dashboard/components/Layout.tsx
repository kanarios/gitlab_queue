...
import { LayoutDashboard, History, BarChart2, GitMerge, Activity, Sun, Moon, ChevronLeft, ChevronRight, Menu, X } from 'lucide-react';
    ...
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 overflow-hidden font-sans transition-colors duration-300">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 z-30 flex items-center justify-between px-4 transition-colors duration-300">
        <div className="flex items-center space-x-2">
            ...
            <span className="font-bold text-lg tracking-tight text-slate-900 dark:text-white">MergeBot</span>
        </div>
        <button 
      ...
      >
        ...
        <button 
          ...
          className="hidden md:block absolute -right-3 top-8 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-full p-1 text-slate-500 hover:text-slate-700 dark:hover:text-slate-200 shadow-sm z-50"
        >
        ...
        <div className={`hidden md:flex p-6 items-center ${isSidebarCollapsed ? 'justify-center' : 'space-x-3'} border-b border-slate-200 dark:border-slate-800`}>
          ...
          {!isSidebarCollapsed && (
            <div className="overflow-hidden whitespace-nowrap">
              <h1 className="font-bold text-lg tracking-tight text-slate-900 dark:text-white">MergeBot</h1>
              <p className="text-xs text-slate-500 font-mono">v1.2.0 • Active</p>
            </div>
          )}
        </div>
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
          {navItems.map((item) => (
            <button
              ...
              className={`flex items-center ${isSidebarCollapsed ? 'justify-center md:px-0' : 'space-x-3'} w-full p-3 rounded-lg transition-all duration-200 ${
                currentView === item.id
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 space-y-4">
           ...
           <button 
             onClick={toggleTheme}
             className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center md:px-0' : 'space-x-3'} p-3 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors text-slate-500 dark:text-slate-400`}
           ...
           <div className={`flex items-center ${isSidebarCollapsed ? 'justify-center md:px-0' : 'space-x-3'} p-3 rounded-lg bg-slate-100 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800`}>
              ...
              {(!isSidebarCollapsed || isMobileMenuOpen) && (
                <div className="text-xs overflow-hidden">
                   <p className="text-slate-600 dark:text-slate-300 font-medium">System Healthy</p>
                   <p className="text-slate-500">Redis Connected</p>
                </div>
              )}
           </div>
        </div>
      ...
      {/* Main Content */}
      <main className="flex-1 overflow-y-auto bg-slate-50 dark:bg-slate-900 relative transition-colors duration-300 pt-16 md:pt-0">
