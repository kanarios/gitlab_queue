...
class Scenario(vedro.Scenario):
    ...
    async def given_mixed_queue(self):
        ...
        await self.queue.add_to_queue(99999, regular1, is_hotfix=False)
        await asyncio.sleep(0.01)

        hotfix1 = create_test_mr(iid=2, title="Hotfix 1")
        await self.queue.add_to_queue(99999, hotfix1, is_hotfix=True)
        await asyncio.sleep(0.01)

        regular2 = create_test_mr(iid=3, title="Regular 2")
        await self.queue.add_to_queue(99999, regular2, is_hotfix=False)
        await asyncio.sleep(0.01)

        hotfix2 = create_test_mr(iid=4, title="Hotfix 2")
        await self.queue.add_to_queue(99999, hotfix2, is_hotfix=True)
