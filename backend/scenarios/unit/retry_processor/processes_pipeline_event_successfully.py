"""Test _process_retry_item marks success for pipeline events."""
...
class Scenario(vedro.Scenario):
    subject = "_process_retry_item marks success for pipeline event"

    def given_processor_and_pipeline_retry_item(self):
        self.processor = create_test_retry_processor()
        self.item = create_test_retry_item(event_type="pipeline")

    async def when_process_retry_item_is_called(self):
        with patch("gitlab_queue.webhooks.handlers.PipelineWebhookHandler") as mock_handler_cls:
