"""Test timeout last-chance check rescues canceled pipeline.

When timeout is reached but pipeline has terminal status (canceled/failed),
the handler gives one extra iteration instead of triggering timeout,
allowing the pipeline status to be properly processed.
"""
...
class Scenario(vedro.Scenario):
    subject = "timeout last chance rescues canceled pipeline"
    ...
    def and_timeout_was_not_triggered(self):
        assert len(self.sm.timeout_calls) == 0

    def and_last_chance_flag_is_set(self):
        assert self.state.timeout_last_chance_used is True

    def and_pipeline_status_was_fetched(self):
        assert len(self.gitlab_client.get_latest_pipeline_calls) == 1
