...
def create_mock_settings(**overrides: object) -> FakeSettings:
    """Create FakeSettings with optional field overrides."""
    return FakeSettings(**overrides)


def create_mock_gitlab_client() -> FakeGitLabClient:
    """Create a FakeGitLabClient for tests."""
    return FakeGitLabClient()


def create_mock_queue_manager() -> FakeQueueManager:
    """Create a FakeQueueManager for tests."""
    return FakeQueueManager()


def create_mock_notifier() -> FakeNotifier:
    """Create a FakeNotifier for tests."""
    return FakeNotifier()


def create_pipeline_wait_state(
    rebase_handler: FakeRebaseDuringTestingHandler | None = None,
