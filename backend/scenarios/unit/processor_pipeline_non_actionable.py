...
async def process_mr_with_non_actionable_pipeline_status(status: str):
    ...
    with when("processor handles the pipeline status"):
        result = await processor._handle_pipeline_status(ctx, sm, pipeline, retry_count=0, max_retries=3)
...
async def non_actionable_status_does_not_retry():
    ...
    with when("processor handles the pipeline status"):
        result = await processor._handle_pipeline_status(ctx, sm, pipeline, retry_count=0, max_retries=5)
