"""An incomplete project-scoped schema must not be stamped as f1."""
...
class Scenario(vedro.Scenario):
    subject = "reject incomplete f1 legacy schema without stamping"

    def given_f1_database_missing_processing_attempts(self):
        self._tmp_dir = tempfile.TemporaryDirectory()
        self._db_path = Path(self._tmp_dir.name) / "incomplete_f1.db"
    ...
    async def when_legacy_stamping_is_attempted(self):
        ...
        with sqlite3.connect(self._db_path) as connection:
            connection.execute("ALTER TABLE merge_requests DROP COLUMN processing_attempts")
        ...
        try:
            ...
            raise AssertionError("Incomplete f1 schema was stamped")
    ...
    def then_it_should_report_the_missing_column_without_stamping(self):
        assert "processing_attempts" in self.error
