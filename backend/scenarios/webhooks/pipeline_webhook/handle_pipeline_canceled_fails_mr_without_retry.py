"""Test: handle pipeline canceled marks MR as failed (processor handles retries)."""
...
class Scenario(vedro.Scenario):
    subject = "handle pipeline canceled marks MR as failed"

    def given_handler_and_event(self):
        self.settings = create_mock_settings()
        self.gitlab_client, self.transport = create_gitlab_client_with_transport()
        self.queue_manager = create_mock_queue_manager()
        self.queue_manager.add_item(create_queue_item_in_state("testing", retry_count=2, mr_iid=123))
        ...
        self.event = create_pipeline_event(mr_iid=123, status="canceled")
