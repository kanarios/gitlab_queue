"""Test that POST /api/dlq/{entry_id}/retry returns 404 when entry not found."""
...
class Scenario(vedro.Scenario):
    subject = "retry DLQ entry returns 404 when not found"

    def given_app_with_missing_dlq_entry(self):
        self.app, self.state = created_test_app()
        self.state.retry_manager.retry_dlq_entry = AsyncMock(
            side_effect=DLQItemNotFoundError(1),
        )
        self.client = TestClient(self.app, raise_server_exceptions=False)
        self.token = created_test_jwt(self.state.settings)
        self.headers = {"Authorization": f"Bearer {self.token}"}

    def when_nonexistent_dlq_entry_is_retried(self):
